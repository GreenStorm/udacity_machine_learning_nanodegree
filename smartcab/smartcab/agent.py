...
class LearningAgent(Agent):
    ...
    def get_max_Q(self, state):
        ...
        Q_state_entry = self.Q[state]
        actionKeys = Q_state_entry.keys()
        highestQActions = [actionKeys[0]] #Will introduce a problem if it's the highest but will be solved by turning it into a set.
        for action in actionKeys:
            if Q_state_entry[action] > Q_state_entry[highestQActions[0]]:
                highestQActions = [action]
            elif Q_state_entry[action] == Q_state_entry[highestQActions[0]]:
                highestQActions.append(action)

        highestQActions = list(set(highestQActions)) #To make it unique actions only
        maxQ = self.randomly_select_action(highestQActions)
        return maxQ 
    ...
    def choose_action(self, state):
        ...
        elif np.random.choice(2,p=[1.0-self.epsilon,self.epsilon]) > 0: # Will output a choice 0 with 1-epsilon chance or choice 1 with an epislon chance
            action = self.randomly_select_action(self.valid_actions)
